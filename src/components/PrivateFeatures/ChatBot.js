...
const ChatBot = () => {
    ...
    if ('ADA Package 4' in userProjects) {
        userProjKeys.push('p4');
    }
    if ('Amtrak ERT' in userProjects) {
        userProjKeys.push('ert');
    }
    if ('CBTC Fulton Line' in userProjects) {
        userProjKeys.push('fulton');
    }
    if ('DB Platform Screen Door Pilot Installation' in userProjects) {
